...
class _AgenticAnalysisCard extends StatefulWidget {
  final String patientId;
  const _AgenticAnalysisCard({required this.patientId});

  @override
  State<_AgenticAnalysisCard> createState() => _AgenticAnalysisCardState();
}

class _AgenticAnalysisCardState extends State<_AgenticAnalysisCard> {
  AgenticCareResult? _result;
  bool _loading = false;
  String? _error;
  ...
  Future<void> _runAnalysis() async {
    ...
    try {
      final results = await Future.wait([
        ...
        PatientService.instance.getTodayHealthData(widget.patientId),
        PatientService.instance.getTodaySosCount(widget.patientId),
      ]);
      ...
      final events = <String>[
        if (patient != null) 'Patient age: ${patient.age}',
        if (health != null) ...[
          'Mood: ${health.mood}',
          'Pain level: ${health.painLevel}/10',
          'Medications taken: ${health.medicationsTaken} of ${health.medicationsTotal}',
...
class _AgenticStep extends StatelessWidget {
  final int step;
  final String label;
  final IconData icon;
  final Color color;
